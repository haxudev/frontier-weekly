...
export async function generateMetadata({ params }) {
  ...
  const title = `${week.title} | 前沿今辰观`
  const description = week.excerpt || week.title
  const url = getPageUrl(`/week/${resolvedParams.slug}`, 'zh')
  const ogImageUrl = getOgImageUrl(week.title, 'zh')
  ...
  return {
    ...
    // Open Graph - 用于微信、Facebook、LinkedIn 等
    openGraph: {
      title: week.title,
      description,
      url,
