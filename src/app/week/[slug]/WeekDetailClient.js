...
export default function WeekDetailClient({ week, recentWeeks, currentSlug }) {
  ...
  useEffect(() => {
    ...
    const openPopover = (citeEl) => {
      ...
      // Display format: [number] title - make it clear this is a citation reference
    ...
    const openHoverCard = (citeEl) => {
      ...
      // If we still don't have a title, don't show an empty card.
      if (!title) {
        closeHoverCard()
        return
      }
      ...
      // Show (hidden) first, measure, then position.
